    ...
    public class ApiTests : IClassFixture<CustomFixture>
    {
        ...
        public async Task Scenario4Test()
        {
            ...
            try
            {
                ...
                {
                    Login = client.Login,
                    Password = $"client_password_wrong_{clientId}"
                };
                var authResult = await htppClient.PostAsync("https://localhost:5721/api/v1/client/auth", clientIdentity.SerializeRequest());
                Assert.False(authResult.StatusCode == System.Net.HttpStatusCode.OK);
                ...
                var request = new HttpRequestMessage()
                {
                    Content = content,
                    Headers = {
                        { HttpRequestHeader.Authorization.ToString(), $"Bearer NO TOKEN" },
                        { HttpRequestHeader.ContentType.ToString(), "application/json" },
                    },
                    RequestUri = new Uri("https://localhost:5721/api/v1/message/send"),
                    Method = HttpMethod.Post
                ...
                Assert.False(sendResult.StatusCode == System.Net.HttpStatusCode.OK);

                var messageRepo = _fixture.ServiceProvider.GetRequiredService<Db.Interface.IRepository<Db.Model.Message>>();
                var messageTest = (await messageRepo.GetAsync(new Db.Model.Filter<Db.Model.Message>()
                {
                    Page = 0,
                    Selector = s => s.ClientId == dbClient.Id,
                    Size = 10
                }, CancellationToken.None)).Data.FirstOrDefault();

                Assert.Null(messageTest);

                clientIdentity = new Contract.Model.ClientIdentity()
                {
                    Login = client.Login,
                    Password = $"client_password_{clientId}"
                };
                authResult = await htppClient.PostAsync("https://localhost:5721/api/v1/client/auth", clientIdentity.SerializeRequest());
                Assert.True(authResult.StatusCode == System.Net.HttpStatusCode.OK);
                ...
                sendResult = await htppClient.SendAsync(request);

                Assert.False(sendResult.StatusCode == System.Net.HttpStatusCode.OK);

                messageTest = (await messageRepo.GetAsync(new Db.Model.Filter<Db.Model.Message>()
                {
                    Page = 0,
                    Selector = s => s.ClientId == dbClient.Id,
                    Size = 10
            ...
            finally
            {
                bool stopped = false;
                if (mainProcess != null)
                    stopped = StopProject(mainProcess);
                _output.WriteLine($"Delete directory: {projPath}");
                if(stopped)
                    Directory.Delete(projPath, true);
            }
        ...
        private bool StopProject(Process cmd)
        {
            try
            {
                cmd?.Kill();
